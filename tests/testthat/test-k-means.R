test_that("K-Means works generally", {
  set.seed(123)
  ...
  clustering <- kMeans(data,K=3,tries = 20)
...
test_that("K-Means different K and tries", {
  data <- generateClusterData(n=100,
                              clusters_mean = tibble::tibble('X'=c(0.4,0.4,0.8),
                                                             'Y'=c(0.7,0.1,0.7)),
                              clusters_sd=c(0.01,0.02,0.01),
  ...
  testthat::expect_no_error(kMeans(data,K=1,tries=5))
...
test_that("K-Means distance_methods and p", {
  data <- generateClusterData(n=100,
                              clusters_mean = tibble::tibble('X'=c(0.4,0.4,0.8),
                                                             'Y'=c(0.7,0.1,0.7)),
                              clusters_sd=c(0.01,0.02,0.01),
...
test_that("K-Means ccustom distance functions", {
...
test_that("K-Means invalid inputs", {
  ...
  # K many clusters must be possible
...
test_that("K-Means misc inputs", {
  generateClusterData()|>
    kMeans(K=4, .print_info = TRUE)|>
    expect_no_error() |>
...
test_that("findClusterAmountElbow works", {
  ...
  expect_equal(findClusterAmountElbow(data,.print_info = TRUE),3) |>
  capture_output(print=FALSE)
...
test_that("findClusterAmountSilhouette works", {
  ...
  expect_equal(findClusterAmountSilhouette(data,.print_info = TRUE),3)|>
    capture_output(print=FALSE)
})

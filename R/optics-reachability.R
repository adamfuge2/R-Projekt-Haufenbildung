...
plot.reachability <- function(x, epsilon=NULL,...){
  ...
  rd_plot <- rd
  max_val <- max(rd[is.finite(rd)], na.rm = TRUE)
...
extractClusters.optics <- function(object, epsilon){
  reach <- as.reachability(object)
  extractClusters(reach, epsilon)
}
...
extractClusters.reachability <- function(object, epsilon) {
  ...
  for (i in seq_len(n)) {
    rd_val <- rd[i]
    cd_val <- if(!is.na(cd[i])) cd[i] else Inf

    if(rd_val > (epsilon + tol)) {
      if(cd_val <= (epsilon + tol)) { # corepoint -> new cluster
        cluster_id <- cluster_id + 1L
        cluster_ordered[i] <- cluster_id
      } else { # no corepoint -> noise
        cluster_ordered[i] <- 0L
      }
    } else {
      if(cluster_id == 0L) { # if still in noise
        cluster_ordered[i] <- 0L
      } else {
        cluster_ordered[i] <- cluster_id
      }
  ...
  if(length(small) > 0L) {
    cluster[cluster %in% as.numeric(small)] <- 0L
  }

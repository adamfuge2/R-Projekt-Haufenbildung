...
generateClusterData <- function(n=100,
  ...
  epsilon <- t(structure(stats::rnorm(n*dim, mean = 0, sd = points_sd),dim=c(dim,n)))
...
generateNoiseData <- function(n=100,
  ...
  data <- 1:dim |> base::sapply(function(i) stats::runif(n,min=lower_bounds[[i]],max = upper_bounds[[i]])) |>

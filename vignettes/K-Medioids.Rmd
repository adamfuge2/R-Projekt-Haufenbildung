...
### 2.4 p
If you chose 'Lp' for the parameter distance (2.3) then this is where you can input the value for the p-metric. Will be ignored, if other distances are chosen.

Examples:
```{r}
set.seed(123)
clustering_1 <- kMedioids(data_1,K=5, distance = 'Lp', p = 1)
set.seed(123)
clustering_2 <- kMedioids(data_1,K=5, distance = 'Lp', p = 2)
set.seed(123)
clustering_3 <- kMedioids(data_1,K=5, distance = 'Lp', p = 4)
```
...
You can also choose a custom distance function. This distance function must be a function with the following properties. If a custom distance function is provided, the parameters distance (2.3) and p (2.4) are ignored. 
